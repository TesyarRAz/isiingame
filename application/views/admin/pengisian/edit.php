...
<div class="d-sm-flex align-items-center justify-content-between mb-4">
    <h1 class="h3 mb-0 text-gray-800">Edit Pengisian</h1>
</div>

<form action="<?= site_url('admin/pengisian/update/' . $id_pengisian) ?>" class="card card-body" method="POST" enctype="multipart/form-data">
    <div class="row">
        <div class="col-md-4">
            <div class="form-group">
                <label class="font-weight-bold" for="kode_pengisian">Kode Pengisian <span class="text-danger">*</span></label>
                <input type="text" class="form-control" name="kode_pengisian" id="kode_pengisian" value="<?= $kode_pengisian ?>" readonly required>
        ...
        <div class="col-md-4">
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="nama_pemesan" id="nama_pemesan" value="<?= $nama_pemesan ?>" required>
            ...
            <div class="form-group">
                ...
                <div class="d-flex align-items-center">
                    <select name="status" id="status" class="form-control" required>
                        <option value="dibuat" <?= $status == 'dibuat' ? 'selected' : '' ?>>Dibuat</option>
                        <option value="diatur" <?= $status == 'diatur' ? 'selected' : '' ?>>Diatur</option>
                        ...
                        <?php foreach ($games as $item) : ?>
                            <tr>
                                ...
                                <div class="form-check form-switch">
                                        <input name="games[]" value="<?= $item['id_game'] ?>" class="form-check-input" type="checkbox" role="switch" id="game-<?= $item['id_game'] ?>" <?php if (in_array($item['id_game'], $ids_game)): ?> checked <?php endif ?>>
